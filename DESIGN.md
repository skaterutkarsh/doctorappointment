...
We rejected Optimistic Locking (versioning) because in a high-contention booking scenario (e.g., ticket release), retrying failures is a bad UX. We want strict serialization.

**Algorithm:**
1.  **Start Transaction.**
2.  **Lock Row:** `SELECT * FROM "Slot" WHERE id = $1 FOR UPDATE`.
    *   This tells Postgres: "Don't let anyone else read/write to this specific row until this transaction finishes."
    *   Other requests requesting this same lock will **WAIT** (queue up) at the database level.
3.  **Check Status:** If `status != AVAILABLE`, rollback and throw error.
4.  **Update Status:** Set status to `BOOKED`.
5.  **Create Booking:** Insert into `Booking` table.
6.  **Commit Transaction.**

This guarantees **ACID compliance** and zero overbooking.
